...
ul=""
ull=[]
easygui.msgbox("Running the Crawler",title="WEB CRAWLER")
easygui.msgbox("Result will be saved to /root/Desktop/result",title="Saving...")
...
with open("/root/Downloads/sub.txt","r") as f:
		...
		ul=options.ul
			...
			f.write(turl)
			f.write("\n")
			print("[+] Discovered URL --> " + turl + "\t\t\t\t\t\t\t" + str(requests.get(turl)))

def crawl(ul):
	easygui.msgbox("Running the Crawler clonner",title="WEB CRAWLER clonner")
	href=url(ul)
	for link in href:
